...
class TestNautilusBacktestEngine:
    ...
    def sample_signals(self, sample_data):
        ...
        np.random.seed(42)
        signals = pd.Series(np.random.choice([True, False], len(sample_data)), index=sample_data.index)
        return signals
    ...
    def test_engine_initialization_with_config(self):
        ...
        config = BacktestConfig(initial_cash=50000.0, commission=0.001, direction="LongOnly")
    ...
    def test_metrics_calculation(self, sample_data, sample_signals):
        ...
        expected_metrics = ["Sharpe", "Sortino", "Calmar", "MaxDrawdown", "TotalReturn", "WinRate", "ProfitFactor"]
    ...
    def test_commission_deduction(self, sample_data, sample_signals):
        ...
        if len(result_no_comm.trades) > 0 and len(result_with_comm.trades) > 0:
            assert result_with_comm.equity_curve["Equity"].iloc[-1] <= result_no_comm.equity_curve["Equity"].iloc[-1]
    ...
    def test_reproducibility_with_seed(self, sample_data):
        ...
        np.random.seed(42)
        signals1 = pd.Series(np.random.choice([True, False], len(sample_data)), index=sample_data.index)

        np.random.seed(42)
        signals2 = pd.Series(np.random.choice([True, False], len(sample_data)), index=sample_data.index)
