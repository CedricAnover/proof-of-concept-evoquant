...
class RSIValue(ParameterBase):
    def __init__(self, in_value, param_name="RSIValue", param_type="Output", str_style=5):
        ...
        if in_value < 0.0 or in_value > 100.0:
            raise ValueError("RSIValue must be within 0 & 100")
    ...
    def __init__(self, in_value, param_name="std", param_type="Input", str_style=1):
        ...
        if not isinstance(in_value, float):
            if (not isinstance(in_value, int)) or isinstance(in_value, bool):
                ...
                self._value = float(in_value)
        if in_value <= 0.0:
            raise ValueError("Std must be more than zero.")
...
class MAMode(ParameterBase):
    ...
    _CTRADER = {"ema": r"MovingAverageType.Exponential", "sma": r"MovingAverageType.Simple"}
    _MT5 = {}
...
def bbands(ser: SeriesFloat, period: Period, std: StdDev, mamode: MAMode, bbout: BBOut) -> SeriesFloat:
    if bbout.value == "bbl":
        return SeriesFloat(
            ta.bbands(ser.to_pd_series(), length=period.value, std=std.value, mamode=mamode.value).iloc[:, 0],
            name="BBandsBBL",
        )
    if bbout.value == "bbm":
        return SeriesFloat(
            ta.bbands(ser.to_pd_series(), length=period.value, std=std.value, mamode=mamode.value).iloc[:, 1],
            name="BBandsBBM",
        )
    if bbout.value == "bbu":
        return SeriesFloat(
            ta.bbands(ser.to_pd_series(), length=period.value, std=std.value, mamode=mamode.value).iloc[:, 2],
            name="BBandsBBU",
        )
    if bbout.value == "bbb":
        return SeriesFloat(
            ta.bbands(ser.to_pd_series(), length=period.value, std=std.value, mamode=mamode.value).iloc[:, 3],
            name="BBandsBBB",
        )
    if bbout.value == "bbp":
        return SeriesFloat(
            ta.bbands(ser.to_pd_series(), length=period.value, std=std.value, mamode=mamode.value).iloc[:, 4],
            name="BBandsBBP",
        )
