...
try:
    ...
    from vectorbt.portfolio.enums import ConflictMode, Direction, DirectionConflictMode, OppositeEntryMode
...
class EvoStrategy(SignalStrategy):
    ...
    start_date = (
        None  # (datetime) Start Date of the Backtest. Important for IS-OOS and Validation. Default=min(df_ohlcv.index)
    )
    end_date = (
        None  # (datetime) End Date of the Backtest. Important for IS-OOS and Validation. Default=max(df_ohlcv.index)
    )
    ...
    def init(self):
        ...
        if self.direction not in ["LongOnly", "ShortOnly", "LongShort"]:
            raise ValueError("Direction has to be given ['LongOnly', 'ShortOnly', 'LongShort'].")
        if self.strategy_name is None or not isinstance(self.strategy_name, str):
            raise ValueError("Strategy Name must be given")
        if self.enable_tsl:
            assert len(self.stop_loss) != 0, "Stop-Loss must be given for Trailing Stop"
        ...
        if self.direction == "LongOnly":
            signal = (
                self.ser_bool.to_pd_series(index=self.data.index).astype(int).diff().fillna(0).replace(-1, 0)
            )  # Converting to Pulse for Long
            if self.exit_encoded_entry:
                self.long_exit = self.I(
                    lambda: extract_start_end_true(
                        self.ser_bool.to_pd_series(index=self.data.index).shift(1).fillna(False), start=False
                    ),
                    plot=False,
                )
        ...
        if self.direction == "ShortOnly":
            signal = (
                self.ser_bool.to_pd_series(index=self.data.index).astype(int).diff().fillna(0).replace(-1, 0).mul(-1)
            )  # Converting to Pulse for Short
            if self.exit_encoded_entry:
                self.short_exit = self.I(
                    lambda: extract_start_end_true(
                        self.ser_bool.to_pd_series(index=self.data.index).shift(1).fillna(False), start=False
                    ),
                    plot=False,
        ...
        if self.direction == "LongShort":
            ...
            if self.exit_encoded_entry:
                self.long_exit = self.I(
                    lambda: (
                        extract_start_end_true(self.ser_bool.to_pd_series(index=self.data.index), start=False)
                        .shift(1)
                        .fillna(False)
                    ),
                    plot=False,
    ...
    def run_exit_when_pnl_lessthan(self):
        ...
        if len(self.trades) > 1:
            raise Exception(
                "The Backtester should only placed 1 trade at a time. There may be something wrong with your backtest settings."
            )
        trade = self.trades[0]
...
def evo_vbt_backtester(
    ...
    vbt.settings.caching["whitelist"].extend(
        [
            vbt.CacheCondition(base_cls="Portfolio", func="get_positions"),
...
def evo_backtester(
    ...
    except AttributeError:  # EntryTime and ExitTime may already be a date.
        pass
    ...
    if use_pct_rets:
        s_rets = res._equity_curve["Equity"].pct_change().fillna(0.0)  # Pct Return Series
    else:  # Use log returns:
        try:
            s_rets = res._equity_curve["Equity"]
            s_rets = np.log(s_rets / s_rets.shift(1)).fillna(0.0)  # Log Return Series
        except ZeroDivisionError:
            # Use Percentage Return if catched ZeroDivisionError.
            s_rets = res._equity_curve["Equity"].pct_change().fillna(0.0)  # Pct Return Series

    # Check if splitter_func is valid
    if splitter_func not in [linear_is_oos, multi_linear_is_oos]:
    ...
    if splitter_func == linear_is_oos:  # ((is_start_date, is_end_date), (oos_start_date, oos_end_date))
        ...
        out_dict["IS"] = (
            s_rets.loc[ind_is],
            df_trades[(df_trades["EntryTime"] >= tup_is[0]) & (df_trades["EntryTime"] < tup_is[1])],
        )
    ...
    if (
        ...
        out_dict["IS"] = [
            ...
            pd.DataFrame(),
        ]  # Create empty Series & DataFrames, we will append items in the loop.
        out_dict["OOS"] = [
            pd.Series(dtype=float),
            pd.DataFrame(),
        ]  # Create empty Series & DataFrames, we will append items in the loop.
        ...
        for _, split_dict in ml_isoos_dict.items():
            for mode, tup in split_dict.items():  # e.g. ("IS", (start, end)) or ("OOS", (start, end))
                if mode == "IS":
                    out_dict[mode][0] = pd.concat(
                        [out_dict["IS"][0], s_rets.loc[tup[0] : tup[1]]], ignore_index=False
                    )  # Series Return
                    out_dict[mode][1] = pd.concat(
                        [
                            out_dict["IS"][1],
                            df_trades[(df_trades["EntryTime"] >= tup[0]) & (df_trades["EntryTime"] <= tup[1])],
                        ],
                        ignore_index=False,
                    )  # Trades DataFrame
                if mode == "OOS":
                    out_dict[mode][0] = pd.concat(
                        [out_dict["OOS"][0], s_rets.loc[tup[0] : tup[1]]], ignore_index=False
                    )  # Series Return
                    out_dict[mode][1] = pd.concat(
                        [
                            out_dict["OOS"][1],
...
def evo_filter_layer1(ser_rets: pd.Series, df_trades: pd.DataFrame) -> bool:
    ...
    try:
        ...
        if not QUANTSTATS_AVAILABLE:
            ...
            outliers = quantstats.stats.outliers(ser_rets, quantile=0.99)  # pd.Series
...
def evo_filter_layer2(bt_res: dict[str, tuple[pd.Series, pd.DataFrame]], filter_list) -> bool:
    ...
    if QUANTSTATS_AVAILABLE:
        ...
        def _fallback_sharpe(rets):
            return 0.0

        def _fallback_sortino(rets):
            return 0.0

        def _fallback_calmar(rets):
            return 0.0

        def _fallback_volatility(rets):
            return rets.std()

        def _fallback_var(rets):
