...
class NautilusMetricsCalculator(MetricsCalculator):
    ...
    def calculate(self, returns: pd.Series, trades: List[Trade]) -> Dict[str, float]:
        ...
        return {
            ...
            "ProfitFactor": float(profit_factor) if not np.isinf(profit_factor) and not np.isnan(profit_factor) else 0.0,
        }
    ...
    def _simulate_trading(
        ...
        for i in range(len(data)):
            ...
            if position_size != 0 and self.config.stop_loss:
                ...
                if sl_unit == "Percent":
                    sl_price = entry_price * (1 - sl_value / 100) if position_size > 0 else entry_price * (1 + sl_value / 100)
                else:
        ...
        equity_curve = pd.DataFrame(
            {'Equity': equity_values, 'DrawdownPct': [0.0] * len(equity_values)},
            index=dates
        )
