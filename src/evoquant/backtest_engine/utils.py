...
def extract_start_end_true(ser: pd.Series, start=True) -> pd.Series:
    ...
    if start:
        return ser.mask(s - s.shift(1).fillna(0.0) <= 0, False)
    else:
        return ser.mask(s.shift(-1).fillna(0.0) - s >= 0, False)
    ...
    for year in years:
        if mode == "week":
            for week_num in week_nums:
                temp_s = in_df.loc[
                    ((in_df.index.year == year) & (in_df.index.isocalendar().week == week_num)), :
                ].index.to_series()
                if len(temp_s) < 1:
                    continue
                else:
