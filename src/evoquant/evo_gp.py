...
def _save_result_to_pickle(directory, file_extension="pickle"):
    def decorator_func(func):
        def wrapper_func(*args, **kwargs):
            ...
            filename = os.path.join(directory, rf"""strategies_{timestamp}_{unique_id}.{file_extension}""")
...
def generate_3permutations(my_list):
    ...
    permutations_list = (
        [(x, y, z) for x, y, z in combinations_list]
...
def is_fitness_tuple_valid(in_tup: tuple | list) -> bool:
...
def evo_mutation(
    ...
    ind = copy.deepcopy(
        individual
    )  # Deepcopy the individual to be mutated. We don't want to modify the original input.
...
def evo_cross(
    parent1: gp.PrimitiveTree, parent2: gp.PrimitiveTree, mutator: callable, termpb: float = 0.5, *args, **kwargs
):
...
def evo_varor(population, toolbox, lambda_, cxpb, mutpb):
    ...
    for _ in range(lambda_):
        ...
        if op_choice < cxpb:  # Apply crossover
            ind1, ind2 = [
                toolbox.clone(i) for i in toolbox.select(population, 2)
            ]  # We use our select method rather than pure random
...
class PerfStats:
    ...
    _PS = {
        ...
        "Avg$Loss": lambda df: df["PnL"].where(df["PnL"] < 0.0).mean(),
        "Avg$Profit": lambda df: df["PnL"].where(df["PnL"] > 0.0).mean(),
    ...
    _fitness_targets: list | tuple = None
    ...
    def fitness_targets(self, in_fitness_targets: list | tuple):
...
def evo_evaluator(
    ...
    perf_stats.set_fitness_required_args(
        evo_bt_res["IS"][0], evo_bt_res["IS"][1]
    )  # Modify Return Series, Trades DataFrame, and Fitness Values in perf_stats for the individual
...
def transform_to_single_objective(
    fitness_vals: tuple[float], weights: tuple[float], method="weighted_sum", fitness_idx=None
) -> float:
    ...
    if method == "select_fitness":
        if fitness_idx is None or not isinstance(fitness_idx, int):
            raise ValueError(
                "select_fitness method requires a valid index. Take a look at the how many fitness functions to optimize."
            )
        if fitness_idx < 0 or fitness_idx >= len(weights):
            raise ValueError(
                "fitness_idx must be within the index range of the length of the given fitness values or weights."
            )
...
def gp_main_algo_random(
    ...
    if (
        isinstance(concurrent_exec_params["n_processors"], int)
        and concurrent_exec_params["n_processors"] > multiprocessing.cpu_count()
    ):
    ...
    for gen in range(n_gen):  # from 0 to n_gen-1
        ...
        if is_single_fitness:  # One Fitness Value
            ...
            if optim_direction == "max":
                subpop = sorted(
                    subpop,
                    key=lambda ind: transform_to_single_objective(
                        ind.fitness.values, perf_stats.weights, method=single_fitness_method, fitness_idx=fitness_idx
                    ),
                    reverse=False,
                )
            if optim_direction == "min":
        ...
        try:
            prev_optimal_ind = subpop[
                -1
            ]  # Save the current optimal individual at gen=0 for comparing with optimal of next generation
        ...
        if subpop_size == 0:
            ...
            count_no_valid_pop = (
                0  # When we generate valid pop (i.e. we have at least 1 valid individual), then reset the counting.
            )
...
def gp_main_algo_standard_gp(
    ...
    if use_concurrent:
        ...
        toolbox.register(
            "map", executor.map, timeout=None, chunksize=chunk_size
        )  # Temporarily register the executor.map in toolbox
    else:
    ...
    if len(in_population) == 0:
        ...
        while True:
            try:
                ...
                population = list(
                    toolbox.map(toolbox.eval_modifier, population)
                )  # It would apply toolbox.evaluate and modify the individual's fitness.values property
                population = [
                    ind for ind in population if is_fitness_tuple_valid(ind.fitness.values)
                ]  # There is a possibility that this list can be empty
    ...
    for gen in range(1, n_gen + 1):
        ...
        while True:
            ...
            offspring = evo_varor(
                population, toolbox, lambda_=n_children, cxpb=cx_pb, mutpb=mut_pb
            )  # Only Crossover and Mutation OR reproduction
            offspring = list(
                toolbox.map(toolbox.eval_modifier, offspring)
            )  # It would apply toolbox.evaluate and modify the individual's fitness.values property
            offspring = [
                ind for ind in offspring if is_fitness_tuple_valid(ind.fitness.values)
            ]  # There is a possibility that this list can be empty
...
def gp_main_algo_multi_islands(
    *standard_gp_args, n_demes: int, n_migrants: int, init_n_gen=4, migration_rate=3, **standard_gp_kwargs
) -> list[gp.PrimitiveTree] | list[None]:
    import concurrent.futures
    import copy
    import multiprocessing
    ...
    assert n_demes <= multiprocessing.cpu_count(), (
        f"The Number of Demes cannot be more than the cpu logical processors in this computer: {multiprocessing.cpu_count()}"
    )
    ...
    main_algo_init_gen = partial(
        _main_algo, standard_gp_args=standard_gp_args, ngen=init_n_gen, standard_gp_kwargs=standard_gp_kwargs
    )
    main_algo_n_gen = partial(
        _main_algo, standard_gp_args=standard_gp_args, ngen=1, standard_gp_kwargs=standard_gp_kwargs
    )
    ...
    for gen in range(1, n_gen + 1):
        ...
        if (
            gen % migration_rate == 0
        ):  # If the current generation is divisible by <migration_rate>, then perform migration
    ...
    if standard_gp_args[1] == "max":
        if len(w) == 1:
            ...
            return sorted(
                out_population,
                key=lambda ind: transform_to_single_objective(
                    ind.fitness.values, w, method=single_fitness_method, fitness_idx=single_fitness_idx
                ),
                reverse=False,
            )
