...
namespace cAlgo.Robots
{
    ...
    public class SPYProbeBot : Robot
    {
        [Parameter(DefaultValue = SPYProbe)]
        ...
        {   
            ...
            BotName = BotName;
            Direction = TradeDirection.LongOnly;
            ExitEncodedEntry = true;
            ExitAfterNBars = null;
            ExitAfterNDays = null;
            ExitEndOfWeek = false;
            ExitEndOfMonth = false;
            ExitWhenPnLLessThan = null;
            ...
            TradeSize = 0.99; //TODO: TradeSize need to be given as Valid Double and converted to volume/units (not lots).
        ...
        {   
            ...
            // Simple Indicators
var indicator0 = Indicators.SimpleMovingAverage(Close, 10).Result;
var indicator1 = Indicators.SimpleMovingAverage(Close, 20).Result;

// Simple Signals
var signal0 = (indicator0 > indicator1 && indicator0.Last(1) < indicator1.Last(1));

// Root Signal
var root_signal = signal0;
            ...
            switch (Direction)
            {
                ...
                if (countPositions > 0) // Return if we have existing long position.
                {
                    ...
                    if(ExitEncodedEntry & !(indicator0 > indicator1 && indicator0.Last(1) < indicator1.Last(1))) // ExitEncodedEntry is Enabled
                    {
                        // Close the position
                        positions[0].Close();
                        break;
                ...
                if ((indicator0 > indicator1 && indicator0.Last(1) < indicator1.Last(1))) // True for Long Signal
                {
                ...
                if (countPositions > 0)
                {
                    Debug.Assert(countPositions == 1, "There is more than 1 position.");
                    // If the root signal is false, check if ExitEncodedEntry is enabled or disabled
                    if(ExitEncodedEntry & !(indicator0 > indicator1 && indicator0.Last(1) < indicator1.Last(1))) // ExitEncodedEntry is Enabled and (indicator0 > indicator1 && indicator0.Last(1) < indicator1.Last(1)) is false
                    {
                        // Close the position
                        positions[0].Close();
                        break;
                ...
                Debug.Assert(countPositions == 0, "There is at least 1 position and didn't exit the method");
                if ((indicator0 > indicator1 && indicator0.Last(1) < indicator1.Last(1))) // True for Short Signal
                {
                ...
                if (countPositions > 0) // If there is an existing position (long or short)
                {
                    ...
                    {
                        // Check the signals
                        if((indicator0 > indicator1 && indicator0.Last(1) < indicator1.Last(1))) // True for Long
                        {
